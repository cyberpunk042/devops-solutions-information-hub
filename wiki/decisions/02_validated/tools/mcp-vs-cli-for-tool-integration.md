...
**Quantified degradation curve** (from src-claude-code-accuracy-tips): Accuracy is observed by one practitioner to degrade at higher utilization (they reported rough markers at 40%, 60%, 80% — but degradation is probabilistic, not deterministic, and well-managed sessions can work effectively at high utilization). MCP schema overhead shifts the starting point upward before any real work begins. CLI starts at zero.
...
> This decision was calibrated for a single-operator workflow where context window efficiency is the primary concern. At different profile levels:
