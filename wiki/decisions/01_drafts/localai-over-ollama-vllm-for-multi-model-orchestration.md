...
sources:
  - id: aicp-compose
    ...
    description: "LocalAI v4.1.3 container config with watchdog + single-active + embeddings pool (AICP project)"
  - id: aicp-models
    type: directory
    file: config/models/
    description: "14 model YAML configs (chat + embed + rerank + vision + STT + TTS + image-gen) all consumed by LocalAI's single OpenAI-compatible API (AICP project)"
  - id: aicp-localai-backend
    type: file
    file: aicp/backends/localai.py
    description: "AICP's LocalAI HTTP client — assumes OpenAI-compatible chat completions + embeddings + audio endpoints (AICP project)"
  - id: model-local-ai
    type: wiki
  ...
  - id: aicp-contribution-staging
    type: wiki
    file: raw/articles/from-aicp/decisions/01_drafts/localai-over-ollama-vllm-for-multi-model-orchestration.md
    description: "Original AICP submission 2026-04-18, staged in raw/ before ingestion here"
tags: [decision, localai, ollama, vllm, runtime, infrastructure, aicp, transferable, ecosystem]
contributed_by: "aicp"
...
> [!warning] Rejected on two grounds: (a) same one-model-per-instance problem as vLLM — doesn't fit single-active-backend. (b) TGI is heavily Docker/GPU-first; AICP's CPU fallback path (phi-2, nomic-embed) benefits from LocalAI's uniform handling of GPU vs CPU models in the same API, which TGI does not prioritize.
...
> [!warning] Rejected: reinventing what LocalAI provides. LocalAI v4.x IS this orchestrator, maintained by a community, OpenAI-compatible, battle-tested. AICP's mission is NOT "build a local inference runtime" — it is "orchestrate AI backends" at a higher level. Building custom glue duplicates what LocalAI does well and diverts effort from AICP's actual mission (progressive offload from Claude to local). Per AICP's CLAUDE.md key principle: "Keep v1 simple and usable. Add complexity only when it earns its place."
...
>
> 2. **Single-active backend under VRAM ceiling is the operational reality.** 8 GB VRAM → 19 GB dual-GPU (2026-04-17 unlock) forces `LOCALAI_SINGLE_ACTIVE_BACKEND=false` + `LOCALAI_MAX_ACTIVE_BACKENDS=3` — LRU eviction across GPU model + embed (CPU) + reranker (CPU). Dual-GPU profile (`config/profiles/dual-gpu.yaml`) now lets the 30B MoE run while still swap-eviction-friendly. LocalAI's MAX_ACTIVE_BACKENDS is the exact knob needed; Ollama does not have a semantic equivalent.
>
...
This decision is **AICP-specific but transferable in shape**. Any project facing the same constraint set (multi-capability local inference under a VRAM ceiling with swap-on-demand) would reach the same conclusion. Projects that need only chat (no embeddings / vision / audio) or that can afford one-model-per-instance (no VRAM ceiling) would reach different conclusions. The decision therefore is in the brain's decisions catalog as an **evidence-backed instance of a runtime-selection decision** — consumers can apply the same rationale framework (capability-types × VRAM-constraint × swap-pattern × reliability-needs) to their own context.

## Relationships

- BUILDS ON: [[model-local-ai|Model — Local AI ($0 Target)]]
- RELATES TO: [[profile-as-coordination-bundle|Profile as Coordination Bundle]] (dual-gpu profile couples LocalAI-specific envs CONTEXT_SIZE + LLAMACPP_PARALLEL with the 19 GB hardware reality)
- RELATES TO: [[skills-as-primary-extension-pattern|Decision — Skills as Primary Extension Pattern]] (sibling AICP decision — both concern AICP's foundational architectural shape)
