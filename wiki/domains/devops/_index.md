...
- [Dual-CCD Cache Partitioning (Zen 5 9900X)](concept-dual-ccd-cache-partitioning-9900x.md) — The **AMD Ryzen 9 9900X** uses a dual-CCD (Core Complex Die) layout: 12 cores split as 2 × 6-core CCDs, each with **i...
- [VFIO GPU Isolation with AMD IOMMU](concept-vfio-gpu-isolation-amd-iommu.md) — **VFIO (Virtual Function I/O) with AMD IOMMU** is the Linux kernel mechanism for binding a physical PCIe device to a ...
- [ZFS Tiered Storage for LLM Inference](concept-zfs-tiered-storage-llm-inference.md) — **ZFS tiered storage for LLM inference** is the pattern of partitioning a single physical NVMe pool into multiple ZFS...
...
`devops`, `openarms`, `openfleet`, `sain-01`, `epic`, `module`, `task`, `readiness`, `quality-gates`, `methodology`, `backlog`, `hierarchy`, `status-propagation`, `wiki-backlog`, `task-management`, `decomposition`, `upward-aggregation`, `cpu`, `amd`, `zen5`
