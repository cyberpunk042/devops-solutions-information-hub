---
title: SDLC Customization Framework — Phases, Scale, and Profile Selection
aliases:
...
sources:
  - id: operator-sdlc-vision
    ...
    description: Operator vision for SDLC customization — project phases, scale tiers, three SDLC profile types
...
tags: [sdlc, customization, framework, phases, scale, profile-selection, methodology, project-lifecycle, cmmi, lean-startup, agentic-sdlc]
---
...
- **Three profiles cover the spectrum.** Simplified (short loops, minimal documents, fast iteration), Middle Ground (stage-gated with selected artifacts, the default), Full (complete artifact chain, all stages, formal reviews, compliance documentation). Each SDLC profile is a complete methodology configuration — not a degraded version of Full but a purpose-designed configuration.
...
### Dimension 3: Three SDLC Profiles
...
> [!warning] The Middle Ground Is the Default — Not the Full Profile
>
> Most projects should start at the middle ground. Full profile is for production systems that have earned it through scale and complexity. Simplified is for exploration that accepts throwaway risk. Choosing Full for a POC is as wrong as choosing Simplified for production. The default exists because most work lives in the middle.

### How This Relates to Existing Methodology Models

The current methodology.yaml has 9 models (feature-development, research, bug-fix, etc.). These models define WHAT stages a task goes through. The SDLC profiles define HOW MUCH process wraps around those stages.

A feature-development task in Simplified profile: document (brief) → implement → test (basic).
The same task in Full profile: document (requirements spec + infrastructure analysis + gap analysis) → design (design plan + decisions + tech spec) → scaffold (types + stubs) → implement (logic, wired into runtime) → test (full assertions + pipeline + review).

The models and profiles are ORTHOGONAL dimensions. Every model × profile combination is valid.

### Interaction: Phase × Scale × Profile Selection

> [!abstract] Recommended SDLC Profile by Phase × Scale
>
...
>
> Profile transitions: use Simplified until it hurts, upgrade to Middle when agent violations or manual fixes become frequent, upgrade to Full when compliance, traceability, or team scale demand it.

### External Research: How Industry Handles Scale and Phase

> [!abstract] CMMI Maturity Levels — Process Rigor Scaling
>
> | CMMI Level | Name | Process State | Analogous Profile |
> |-----------|------|---------------|----------------|
> | 1 | Initial | Ad hoc, chaotic. Success depends on heroics. | No profile (chaos) |
...
>
> Key Lean principle: "Limit the scope of change as the product matures." This IS our phase progression — POC allows drastic changes, Production requires stability. The SDLC profile naturally tightens as the product matures.
>
...
>
> This validates our approach: methodology models (stage-gated, artifact-producing) PLUS enforcement infrastructure (hooks, harness, immune system) PLUS the SDLC profile selection (how much process wraps the models). Traditional SDLC doesn't account for agents. Our framework does.
>
...
> | Direction | Go To |
> |-----------|-------|
> | **What identity determines my profile?** | [[project-self-identification-protocol|Project Self-Identification Protocol — The Goldilocks Framework]] — 7 questions → profile selection |
> | **What enforcement matches each profile?** | Simplified → L1 advisory. Default → L2 hooks+commands. Full → L3 immune system+fleet. See [[three-pm-levels|Three PM Levels — Wiki to Fleet to Full Tool]] |
> | **How does readiness/progress work per profile?** | [[readiness-vs-progress|Readiness vs Progress — Two-Dimensional Work Tracking]] — both dimensions at every level, gates tighter on full profile |
> | **What global standards validate this?** | CMMI Levels 1-5 map to our profiles. Lean Startup BML maps to our phases. See [[src-sdlc-frameworks-research|Synthesis — SDLC Frameworks Research — CMMI, Lean Startup, and Agentic SDLC]] |
...
> | **What methodology models work per profile?** | [[model-methodology|Model — Methodology]] — all 9 models work in all profiles, but profile determines artifact depth per model |
> | **Where is the backlog hierarchy?** | [[backlog-hierarchy-rules|Backlog Hierarchy Rules]] — Milestone→Epic→Module→Task. Full profile uses all 4 levels. Simplified may skip milestones+modules. |

## Open Questions

> [!question] Should SDLC profile selection be per-project or per-task? **PARTIALLY RESOLVED**
> Yes — per-task within a project default. A production project (Full profile default) doing a hotfix uses Hotfix model (2 stages, minimal process) — this IS simplified profile applied to one task. The methodology model already handles this. The project profile sets the DEFAULT; the model can override downward (never upward). Remaining: formalize the override rules in methodology.yaml.

> [!question] ~~How do SDLC profiles map to the wiki's 4-tier adoption guide?~~
> **RESOLVED:** Parallel dimensions. Tiers = enforcement depth (read→configure→validate→enforce). Profiles = process weight (simplified→default→full). Pairings: Simplified+Tier1-2, Default+Tier2-3, Full+Tier3-4. See [[methodology-adoption-guide|Methodology Adoption Guide]].
