...
- **Scales down from datacenter GPUs to consumer hardware.** Block-diffusion drafts deliver ~2× speedup on Qwen3.6-27B running on a single RTX 3090 (24GB) — meaning the technique is not exclusive to B200/H100-class hardware. Relevant for sovereign / local-first deployments where the GPU is a 24GB consumer card. ([[src-dflash-block-diffusion-spec-dec|DFlash synthesis]])
...
| **Logic Engine (Weaver-adjacent)** | RTX 3090 (24GB) | Parsing, JSON compilation, regex, code completion | **High** — community writeups confirm ~2× on Qwen3.6-27B on 24GB; ideal for structured-output workloads |
...
- Block-diffusion SpecDec on the SAIN-01 dual-GPU topology: how does it interact with `--tensor-parallel-size 2` across the Blackwell + RTX 3090? Single-GPU benchmarks dominate the published numbers. (Requires: deployment test on the dual-GPU topology.)
