...
- Does Obsidian's graph view scale well to thousands of nodes, or does it become visually unusable? (Requires: external benchmarking data or user reports on Obsidian graph performance at 1000+ nodes; the Second Brain Architecture page notes that this wiki uses the graph view for relationship visualization but does not document performance limits)

### Answered Open Questions

**Q: Can Obsidian plugins automate the trigger to Claude Code after a web clip is saved (e.g., auto-ingest on new file in raw)?**

Cross-referencing `WSL2 Development Patterns` and `Second Brain Architecture`: yes, and the mechanism is already built into the ecosystem's tooling. The `WSL2 Development Patterns` page documents the `wiki-watcher` systemd daemon (tools/watcher.py) which polls the Linux-side wiki/ path for changes — specifically including the `raw/` folder — and triggers the post-chain automatically on file creation or modification. The watcher supports `--watch` mode for continuous daemon operation and `--interval` for configurable polling. The architecture is: (1) Obsidian Web Clipper saves a file to the vault's `raw/` folder on the Windows filesystem, (2) tools/sync.py --reverse syncs that file from Windows back to the WSL2 Linux filesystem raw/ path, (3) wiki-watcher detects the new file and triggers the pipeline. The `Second Brain Architecture` page confirms this is the intended workflow: "A weekly `pipeline gaps` scan and stale-page audit would approximate this. The watcher daemon could automate it." The only gap in the current implementation: the reverse sync (Windows → WSL2) must be either running as a daemon or triggered manually. If the sync daemon's `--watch` mode with `--reverse` flag is enabled, the full clip-to-ingest pipeline is automated end-to-end.

**Q: How does Obsidian handle merge conflicts if the vault is synced across multiple devices while Claude Code is modifying files?**

Cross-referencing `WSL2 Development Patterns`: the WSL2 development patterns page documents the explicit conflict resolution strategy for this ecosystem. The wiki lives on the Linux filesystem (WSL2 ext4) as the source of truth, not on Windows/Obsidian. The `tools/sync.py` uses rsync with the `--update` flag to resolve conflicts by keeping the newer file. The documented workflow assumption is "WSL2 is the source of truth for wiki writes" — Claude Code modifies files on the Linux filesystem, and Obsidian on Windows reads a one-way sync copy. If Obsidian edits are made directly (e.g., adding notes via the Obsidian interface), the `--reverse` flag syncs them back to WSL2, but the `--update` flag means whichever file is newer wins. True bidirectional merge (like Syncthing) is explicitly out of scope as documented in the `WSL2 Development Patterns` page: "True bidirectional merge (like Syncthing) is outside scope — the workflow assumes WSL2 is the source of truth for wiki writes." The practical implication: merge conflicts are prevented by workflow convention (Claude Code writes to Linux, Obsidian is read-only on Windows), not by a conflict resolution protocol. Simultaneous edits from Obsidian and Claude Code on the same file risk the `--update` flag discarding one set of changes based purely on file timestamp.

**Q: Are there Obsidian community plugins specifically designed for the LLM Wiki workflow?**
