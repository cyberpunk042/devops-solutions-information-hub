...
- What is the performance overhead of running 10 agents through a single gateway vs distributed gateways? (Requires: empirical load testing comparing single-gateway vs multi-gateway throughput for 10+ simultaneous agent sessions; the Agent Orchestration Patterns page notes the 10-agent ceiling is documented but does not provide latency or throughput measurements for the gateway serialization bottleneck)

### Answered Open Questions

**Q: Can OpenClaw's plugin hook system be used to integrate the research wiki as a knowledge source for all agents?**

Cross-referencing `OpenArms` and `Four-Project Ecosystem`: yes, and the integration path is documented as already implemented (or implementable without new protocols). OpenClaw's plugin hooks system includes `before_prompt_build` — a hook that fires before context assembly for each agent turn. This is the natural injection point for wiki knowledge: a plugin registered to `before_prompt_build` can query the research wiki's MCP server (via the `wiki_search` or `wiki_read_page` tools) and inject relevant wiki content into the agent's context before the prompt is assembled. The `Four-Project Ecosystem` page documents the wiki's MCP server (15 tools including `wiki_search`, `wiki_read_page`, `wiki_status`) registered in `.mcp.json` and auto-discovered by Claude Code. The `OpenArms` page documents the `mcporter` bridge as the mechanism for connecting MCP servers to gateway-based agent runtimes — the same bridge applies to OpenClaw's gateway. The `OpenFleet` page confirms that `kb_sync.py` already pipes wiki knowledge into the fleet via LightRAG; the plugin hook approach would be a complementary, more granular integration for per-query retrieval rather than bulk graph loading. The practical implementation: a plugin registered to `before_prompt_build` that calls `wiki_search` with the current task's keywords and prepends the top-N results to the prompt context. The `after_tool_call` hook could additionally capture successful tool results and queue them as raw sources for wiki ingestion, closing the knowledge loop from agent execution back to the wiki.

**Q: How does session serialization handle burst dispatch from the orchestrator (multiple tasks in one cycle)?**
