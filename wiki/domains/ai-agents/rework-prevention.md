...
- Is there a way to measure rework rate per agent or per task type empirically, enabling calibrated prevention investment? (Requires: implementation of per-agent rework tracking in OpenFleet's orchestrator; the existing audit ledger tracks task state transitions but does not currently classify which transitions represent rework cycles vs. normal flow)

### Answered Open Questions

**Q: Should the wiki's `smart` ingestion mode learn from past rework instances — tracking which source types or domains triggered rework — to refine its escalation heuristics?**

Cross-referencing `Plan Execute Review Cycle` and `Always Plan Before Executing`: yes, and the mechanism for doing so already exists structurally. The `Plan Execute Review Cycle` page documents that the smart ingestion mode "escalates to guided when: new domain, contradictions detected, ambiguity, low-quality source" — these are static heuristics. The `Always Plan Before Executing` lesson documents that the "guided mode shows an extraction plan and waits for approval before writing any pages — maximum rework prevention." The `Plan Execute Review Cycle` page's analysis of the wiki's ingestion modes notes that the post-chain validate step (exit code 1 on errors) is "the only hard gate in auto mode." The practical path for learning from past rework: the wiki's `## Open Questions` sections that were previously tagged with rework signals (contradictions, domain gaps, thin pages) are exactly the corpus that should inform escalation heuristics. The `pipeline gaps` command already surfaces thin pages and orphans — a `pipeline crossref` pass after every smart-mode ingestion would identify whether the new pages integrated cleanly or created link gaps, serving as a proxy rework signal. Full heuristic learning requires logging which sources triggered manual corrections.

**Q: What is the rework cost model for an incorrect decision in `wiki/decisions/` that affects multiple downstream wiki pages?**

Cross-referencing `Plan Execute Review Cycle` and `Harness Engineering`: the rework cost follows the same compound model as agent task rework but with an additional amplification factor — decision pages are read by downstream pages via `DERIVED FROM` and `BUILDS ON` relationships. The `Plan Execute Review Cycle` page documents that the wiki's multi-pass ingestion means "downstream tasks built on incorrect state must also be reverted." An incorrect decision page that is linked by N downstream pages requires: (1) identifying all N downstream pages via `BUILDS ON` / `DERIVED FROM` traversal, (2) updating or reverting the decision page, (3) reviewing each downstream page for content that was derived from the incorrect decision. The `Harness Engineering` page's answered question on wiki guardrails proposes the prevention mechanism: "block deletion of maturity: canonical pages without explicit override" — the same principle applies to `wiki/decisions/` pages: a decision marked `status: verified` should require explicit confirmation before modification, forcing awareness of downstream impact. The cost amplification factor is the number of BUILDS ON edges pointing to the incorrect decision — a canonical decision with 10 downstream pages costs 10x the rework of a seed decision with 1 downstream page.

**Q: Can post-execution spec compliance review be automated — comparing generated pages against their source material programmatically?**

Cross-referencing `Plan Execute Review Cycle` and `Harness Engineering`: partial automation is feasible using the existing toolchain. The `Plan Execute Review Cycle` page documents that the post-chain's validation step (exit code 1 on errors) enforces schema compliance but not content compliance — it verifies that frontmatter fields are present and valid, not that the page content accurately reflects the source. The `Harness Engineering` page's answered question on wiki guardrails identifies the mechanism: "after any wiki write, run `python3 -m tools.validate` and block completion on non-zero exit." Content-level spec compliance (does the page match the source?) requires comparing the generated `## Summary` and `## Key Insights` against the source file in `raw/`. The `Knowledge Evolution Pipeline` page's post-chain includes a lint step — extending the linter with a "summary faithfulness check" (does the summary contain claims not present in the cited source files?) would automate this. The `Plan Execute Review Cycle` page notes the wiki's guided mode implements manual spec compliance review — automation would need the same inputs (source text + generated page) with an LLM judge comparing them, which is feasible as a `tools/lint.py` extension but requires LLM inference at lint time.
