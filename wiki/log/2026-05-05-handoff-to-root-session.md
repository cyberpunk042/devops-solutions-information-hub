...
All passed `pipeline post` schema gate (0 validation errors). Every lesson has the 4 required schema sections (Context, Insight, Evidence, Applicability) plus Relationships + structured tables.

### Lessons most directly relevant to /root agent's behavioral discipline
...
| `wiki/lessons/03_validated/methodology-process/decision-presentation-discipline-context-guidance-recommendation.md` | SB-071 (15:56) | The DECISION/CONTEXT/GUIDANCE/RECOMMENDATION/ALTERNATIVES/TO ANSWER package format |
| `wiki/lessons/03_validated/methodology-process/structural-but-not-surfaced-failure-mode-fix-must-reach-the-operator-at-use-time.md` | SB-075 (16:46) | Three failure variants: verbal-only, structural-but-not-surfaced, genuine fix |
| `wiki/lessons/03_validated/enforcement-compliance/correction-as-calibration-not-swing-the-going-to-extremes-anti-pattern.md` | "going to extremes" (14:40) | Calibrate-vs-swing test as structural prevention |
| `wiki/lessons/03_validated/enforcement-compliance/refine-triggers-not-revoke-permissions-when-fixing-overzealous-rules.md` | trigger refinement directive | The specific case (rule-edit with permission semantics) |
| `wiki/lessons/03_validated/enforcement-compliance/verbal-acknowledgment-is-not-a-fix-bug-fix-requires-structural-artefact.md` | "do something not just repeat" | Required structural artefact per fix |
| `wiki/lessons/03_validated/methodology-quality/fake-blockers-vs-real-blockers-empirical-verification-required.md` | "retard blockers" + "none of them are blockers" | SRP discipline + empirical verification |
| `wiki/lessons/03_validated/methodology-quality/anti-minimizing-systemic-bug-counts-and-blocker-filter-discipline.md` | "stop minimizing" + blocker-filter | Honest counts + pre-surface filter |
| `wiki/lessons/03_validated/methodology-quality/bug-vs-systemic-bug-nuance-systemic-bugs-live-at-the-meta-level-harness-ecosystem-brain-files.md` | "nuance between bug and systemic bug" | Meta-level vs inside-the-code classification |
| `wiki/lessons/03_validated/methodology-process/sidetrack-detection-and-recovery-when-agent-loses-the-original-task.md` | "how did we go from on track to sidetrack" | Comments-don't-deroute discipline |
| `wiki/lessons/03_validated/methodology-process/compound-waterfall-input-retention-rule-cumulative-not-discarded.md` | compound-waterfall directive | Cumulative input handling + cascade |
...
2. **Skim**: correction-as-calibration-not-swing (meta-pattern; composes with /root's principle #12)
3. **Skim**: multi-branch-parallel-driving (the SB-076 directive captured cross-applicable; useful when listing options)
4. **Skim**: decision-presentation-discipline (the SB-071 package format; load-bearing for any operator-pending surfacing)
...
### Bash multi-line for-loop variable-expansion bug (registered, unreproduced)

Captured in `raw/notes/2026-05-05-bash-multiline-for-loop-variable-expansion-finding-not-reproduced.md`. The original failure (sed received `$f` literally) was not reproducible in isolation despite controlled retests. Hypotheses captured; future-occurrence protocol set. Worth noting in case /root agent encounters something similar.

## Forward suggestions for /root agent (operator-mediated, not directives)

If operator wants /root agent to engage with these on the /root side:
...
- Pattern: [Second-brain observer](../patterns/02_synthesized/second-brain-observer-pattern-extracting-learnings-from-live-sister-project-sessions.md)
- Pattern: [Session-orientation pair](../patterns/03_validated/architecture/session-orientation-pair-sessionstart-hook-and-orient-command-with-orient-report.md)
- Pattern: [Three-mode agent](../patterns/03_validated/architecture/agent-modes-three-mode-pattern-with-mode-aware-loop-cycles.md)
- Adoption decision: [orient pair](2026-05-05-orient-pair-adoption-decision.md)
- Operator transcension directive: [opt-in feature transcension](../../raw/notes/2026-05-05-opt-in-transcension-of-root-features-to-individual-projects-directive.md)
