...
- [Claude Code env-runner re-stages stop-hook-git-check.sh from baked template at every session start](01_drafts/claude-code-env-runner-restages-stop-hook-script-from-baked-template-at-every-session-start.md) — In Claude Code's cloud/remote execution environment, every session-start invocation runs `/opt/env-runner/environment...
...
- [Claude Code synthetic-resume-pair swallows perpetual `/goal` after idle suspend (cloud_default env)](01_drafts/claude-code-synthetic-resume-pair-swallows-perpetual-goal.md) — In `CLAUDE_CODE_REMOTE_ENVIRONMENT_TYPE=cloud_default`, the session auto-suspends after idle (~8 minutes observed)
...
- [Hermetic Testability Is a Precondition for Test-Backfill Scope — Classify Scanners by Input-Source Knob Before Committing to Coverage](01_drafts/hermetic-testability-precondition-gates-test-backfill-scope.md) — When backfilling tests onto a corpus of scanners/validators that read
system or environment state, hermetic testabili...
...
- [Layer-Up Over Crate-Already-Shipped — A Repeatable Promotion Pattern for Partial Milestones](01_drafts/layer-up-over-crate-already-shipped-promotion-pattern.md) — ### TL;DR

When a partial milestone has a shipped Rust crate but no operator-
facing surface, follow this 5-step laye...
...
`lesson`, `layer-2`, `sister-project-applicable`, `contributed`, `methodology`, `inbox`, `quality`, `mission-2026-05-06`, `failure-lesson`, `day-arc-2026-05-08`, `p4-specialization`, `multi-day-pain-point-resolution`, `cross-domain`, `claude-code`, `behave-from-not-over`, `hooks`, `second-brain`, `structural-enforcement-pending`, `openarms`, `enforcement`
