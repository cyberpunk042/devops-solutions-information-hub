...
- [Agents take small unauthorized scope expansions when the change is a 'clean win'](01_drafts/contributed/agents-take-small-unauthorized-scope-expansions-when-the-cha.md) — The v8 methodology blocks overt scope creep through stage hooks, diff validators, and done-when checks
- [Epic readiness math is wrong when an epic has implicit goals beyond its current children](01_drafts/contributed/epic-readiness-math-is-wrong-when-an-epic-has-implicit-goals.md) — The harness computes epic readiness as the average of its child task readiness values
...
- [Right-size the methodology model to the actual work, not the structural category](01_drafts/contributed/right-size-the-methodology-model-to-the-actual-work,-not-the.md) — The methodology selector picks a model based on `task_type` alone
