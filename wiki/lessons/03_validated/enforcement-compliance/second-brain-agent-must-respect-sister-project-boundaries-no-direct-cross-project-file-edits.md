...
sources:
  ...
  - id: companion-rule-root-side
    ...
    description: "Companion to the /root-side rule: /root agent must not write to the second-brain repo directly (operator: 'LET THE SECOND-BRAIN BE ITS OWN'). This lesson is the symmetric inverse: second-brain agent must not write to sister-project files directly."
...
- The agent is operating in the second-brain project ($HOME/devops-solutions-information-hub)
...
>
> The second-brain agent's authority is bounded by the second-brain repo — even when the second-brain is the central knowledge hub for the ecosystem, that does NOT grant it write authority into sister projects. The boundary is structural, symmetric to the /root-side rule *"LET THE SECOND-BRAIN BE ITS OWN"*: each project's agent is the authority on its own files.
...
1. Operator running test session at root-ghostproxy (in /root); operator simultaneously working with second-brain agent (in the second-brain repo) on related observations
...
3. Authored lessons in $HOME/devops-solutions-information-hub/wiki/lessons/01_drafts/ — appropriate scope (registering learnings in second brain)
...
6. Hours of work went into /root file edits when none should have happened from the second-brain repo
...
|--------|----------------------|
| **Second-brain agent (this lesson's home)** | Never edit sister-project files; surface findings as the second-brain repo artifacts; use contribute channel |
...
Operator was running a live test session of `root-ghostproxy` (in /root). Operator was simultaneously working with the second-brain agent (in the second-brain repo) on related observations. The second-brain agent (this agent):
...
2. Authored lessons in $HOME/devops-solutions-information-hub/wiki/lessons/01_drafts/ — appropriate scope (registering learnings in the second brain).
...
- Hours of work went into /root file edits when none should have happened from the second-brain repo
...
- **Read** — any project's agent can READ another project's files for context (the second brain explicitly allows sister projects to read the second-brain repo; the inverse — second-brain reading sister-project files for inputs — is also fine).
...
1. **Stay in the second-brain repo scope** — work on second-brain artifacts only
2. **Author the cross-applicable content** — lessons, patterns, principles in $HOME/devops-solutions-information-hub/wiki/lessons|patterns|spine/
...
| "We need to evolve the rules to prevent this" | "Edit /root/.claude/rules/operating-principles.md" (NO — those are /root's rules) | Author the principle as a the second-brain repo lesson or pattern; sister project pulls when ready (YES) |
| "Move those files from the second-brain repo to /root" | "Run mv across the boundary unilaterally" (NO — touches /root state) | Author the content as second-brain artifacts in the second-brain repo's correct layer; if the operator wants it migrated to /root, /root's agent does the migration (YES) |
| "Update the rule in operating-principles.md" | Assume it's the file path closest to where the work is happening (NO if that's /root and you're the second-brain repo) | Verify which project owns the rule; only edit if it's YOUR project's rule (YES) |

## Relationships

- Companion rule (root-side): `root-ghostproxy/.claude/rules/work-mode.md` — *"DO NOT write to `$HOME/devops-solutions-information-hub/`"*
...
- A sister-project rule appears broken (sister project's agent fixes it, not the second-brain repo's agent)
