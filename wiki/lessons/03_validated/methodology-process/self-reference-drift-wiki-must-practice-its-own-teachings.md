...
> **RESOLVED 2026-04-25** via `gateway compliance --operational` (new flag added 2026-04-25 to `tools/gateway.py`). The flag adds 3 operational checks beyond structural file-presence: (1) hooks operationally wired in `.claude/settings.json` (not just `.claude/hooks/` exists), (2) CLAUDE.md has operational structure (Hard Rules + tables, not just prose pointers), (3) recent validation evidence (`wiki/manifest.json` `generated` timestamp <7 days old). **Post-2026-04-24 refactor measurement: Tier 4/4 structural + 3/3 operational** — genuinely operational. **Pre-refactor (2026-04-24 morning) the same check would have reported 1/3 operational** — failing on hooks (`.claude/hooks/` absent) and CLAUDE.md structure (95-line prose-pointer file with no Hard Rules / routing table). The check is now the deterministic gate that closes the structural-vs-operational gap this lesson identified. Run it: `.venv/bin/python -m tools.gateway compliance --operational`
