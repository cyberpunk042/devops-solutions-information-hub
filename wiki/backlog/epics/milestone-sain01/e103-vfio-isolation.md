...
- [ ] **GRUB updated** with `GRUB_CMDLINE_LINUX_DEFAULT="quiet splash amd_iommu=on iommu=pt kvm_amd.npt=1 kvm_amd.avic=1 vfio-pci.ids=10de:2684,10de:22ba nvidia-drm.modeset=1 nvidia.NVreg_EnableGpuFirmware=1"`; `update-grub` succeeds
...
- [ ] **PCI device IDs verified**: confirm `lspci -nn -s <bdf>` returns the GPU's real id via `lspci -nn` — the `10de:2684`/AD102 value here is the earlier RTX 3090 assumption; the RTX 4090 is AD102 (typically `10de:2684`). Update the GRUB `vfio-pci.ids` line to the actual 4090 GPU + audio ids
...
> - **The PCI device ids `10de:2684` (AD102) + `10de:22ba` were the earlier RTX 3090 (Ampere) values** — the actual card is an RTX 4090 (AD102, GPU id typically `10de:2684`); re-derive both from `lspci -nn` on the installed 4090. Both go in the same `vfio-pci.ids=` argument because they're in the same IOMMU group.
