...
Procure, assemble, and verify the physical hardware substrate for SAIN-01. The platform is deterministic: **AMD Ryzen 9 9900X** (12C/24T, dual-CCD, single-cycle 512-bit AVX-512), **ASUS ProArt X870E-Creator** motherboard, **NVIDIA RTX PRO 6000 Blackwell** (96 GB) + **NVIDIA RTX 3090** (24 GB) in PCIEX16_1 + PCIEX16_2 slots respectively, **256 GB DDR5** (initial 128 GB acceptable; expand later), **2× PCIe 5.0 NVMe** in M.2_1 + a *second* M.2 slot that is NOT M.2_2, **Marvell AQC113C 10GbE** + **Intel I226-V 2.5GbE** on the board. The critical platform constraint: **M.2_2 must remain unpopulated** — populating it triggers PCIe-lane bifurcation that drops PCIEX16_2 to x4, destroying the x8/x8 GPU symmetry the rest of the milestone depends on. This epic ends when the `friction-audit` script (delivered by E101) passes at boot: x8/x8 lane symmetry verified, IOMMU groups clean (Blackwell + 3090 in distinct groups), `cppc` preferred-CCD identified, MOK key generated for Secure Boot signing.
...
- [ ] **PCIe slot assignment correct**: Blackwell in PCIEX16_1, 3090 in PCIEX16_2 — verified before first boot
...
- [ ] **PCIe link width verified** via `lspci -vvv -s <blackwell_bdf>` and `lspci -vvv -s <3090_bdf>` — both show `LnkSta: Speed 16GT/s (ok), Width x8 (ok)` (or PCIe 4.0 x8 acceptable for the 3090 since it's Ampere)
- [ ] **IOMMU groups verified** via `find /sys/kernel/iommu_groups/ -type l | sort` — Blackwell GPU + audio in one group, RTX 3090 GPU + audio in a separate group, no other devices in the GPU groups
