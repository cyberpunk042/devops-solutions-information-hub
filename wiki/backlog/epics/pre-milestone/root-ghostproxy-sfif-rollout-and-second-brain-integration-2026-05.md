---
title: "Epic — root-ghostproxy SFIF Rollout + Second-Brain Integration (2026-05)"
aliases:
  - "Epic — root-ghostproxy SFIF Rollout + Second-Brain Integration (2026-05)"
type: epic
domain: cross-domain
status: draft
confidence: high
...
sources:
  ...
  - id: root-ghostproxy-identity-profile
    type: wiki
  ...
  - id: sister-projects-registry
    ...
    description: "root-ghostproxy entry added 2026-05-04 with type=root, group=operating-system-setup, auto_connect=false"
tags: [epic, root-ghostproxy, sfif, second-brain-integration, sister-project, type-root, group-operating-system-setup, pre-milestone, future-session-work]
---
...
>
> - root-ghostproxy reaches SFIF Infrastructure tier (methodology-aware tooling, validation pipeline, agent harness layer in place)
> - root-ghostproxy reaches SFIF Features tier on at least one specialized OS-setup feature beyond AI-safety hooks (suricata module OR polarproxy module — operator picks which first)
> - root-ghostproxy is connected as a second-brain sister: research-wiki MCP entry installed in its `.mcp.json`, gateway/view forwarders installed in its `tools/`, AGENTS.md has the `## Second Brain Connection` block
> - From a fresh Claude Code session opened in root-ghostproxy, the agent has full operating environment: project-specific CLAUDE.md, universal AGENTS.md, methodology pointer (or local methodology), backlog, log/, hooks, second-brain MCP available
> - Type=root and Group=operating-system-setup are operator-confirmed (or revised) once the rollout has shaken out edge cases
> - sister-projects.yaml `auto_connect` for root-ghostproxy is operator-decided based on the integration outcome

## Done When

> [!info] Verifiable completion criteria
>
> ### Stream 2 — Pure SFIF project base (root-ghostproxy itself)
>
> - [ ] AGENTS.md exists at root-ghostproxy repo root (universal cross-tool context — < 100 lines per three-layer architecture)
> - [ ] CLAUDE.md exists at root-ghostproxy repo root (Claude Code-specific routing — < 200 lines per CLAUDE.md structural patterns standards)
> - [ ] root-ghostproxy README.md updated to document its position as sister project (type=root, group=operating-system-setup, second-brain-connected)
...
>
> - [ ] `python3 -m tools.setup --connect-project <root-ghostproxy-path>` runs from second brain and succeeds
> - [ ] root-ghostproxy/.mcp.json has a `research-wiki` MCP server entry pointing at the second brain's venv + cwd
> - [ ] root-ghostproxy/tools/gateway.py forwarder exists and `python3 -m tools.gateway orient` runs from inside root-ghostproxy
> - [ ] root-ghostproxy/tools/view.py forwarder exists and `python3 -m tools.view spine` runs from inside root-ghostproxy
> - [ ] root-ghostproxy/AGENTS.md contains the `## Second Brain Connection` auto-generated block (see `_install_view_forwarder` + AGENTS.md updater in `tools/setup.py`)
> - [ ] sister-projects.yaml entry's `auto_connect` field flipped to `true` IF operator authorizes (operator-decision)
>
> ### Cross-cutting verification
>
> - [ ] A fresh Claude Code session opened with `claude` in the root-ghostproxy repo orients itself within 60 seconds via CLAUDE.md + gateway orient — no manual intervention
> - [ ] Two-layer hook architecture preserved: machine-level `~/.claude/settings.json` hooks fire BEFORE project-level `<root-ghostproxy>/.claude/` hooks (project may not have a project-level layer initially — that's fine, machine layer suffices)
> - [ ] A worked example: ingest the root-ghostproxy README into the second brain as a source-synthesis page (proves bidirectional knowledge flow works)
...
| Module | Focus |
|---|---|
| **M6 — Pre-connect verification** | Verify Stream 2 M1 (AGENTS.md) is complete; verify root-ghostproxy is at a clean git state; verify operator authorizes the connection |
...
| **M8 — Smoke test from inside** | Open fresh Claude Code session in root-ghostproxy. Run `gateway orient` (should resolve to the connected second brain). Run `view spine` (should print the second brain's spine). |
| **M9 — Worked example** | Ingest root-ghostproxy README as a source-synthesis page in the second brain. Proves the bidirectional flow. |
...
> [!question] Should root-ghostproxy carry its own methodology.yaml or just point to the second brain?
> Per Goldilocks (simplified profile, micro scale), pointing to the second brain is likely the right call — root-ghostproxy doesn't need a full methodology engine on day one. But the project may grow and want its own. Decide in M2. (Requires: operator confirmation)
...
> [!question] Will root-ghostproxy ever auto-connect from `python3 -m tools.setup`?
> `auto_connect: true` means `tools.setup` (no args) will hook it up automatically when the path resolves locally. Given root-ghostproxy is *operating-system-setup* and gates the security envelope, operator may want auto-connect to stay false even after the integration is stable — manual `--connect-project` keeps the operator in the loop. (Requires: operator decision in M10)

> [!question] Should other type=root projects share infrastructure with root-ghostproxy?
> If future projects of type=root + group=operating-system-setup appear (container-runtime-setup, network-edge-setup, etc.), do they share modules / hook patterns / install conventions with root-ghostproxy? Or are they independent? (Requires: when the second project of this group exists)
