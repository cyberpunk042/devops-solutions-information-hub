---
title: "T075 — Author /opt Second-Brain Assistant Profile per M002 schema (canonical tailored example)"
type: task
domain: backlog
status: draft
priority: P1
...
sources:
  - id: parent-module
    type: wiki
    file: wiki/backlog/modules/e024-m003-opt-second-brain-assistant-profile-canonical-example.md
    description: "Parent module E024-M003 — /opt canonical example"
...
# T075 — Author /opt Second-Brain Assistant Profile

## Summary

Author the tailored profile for /opt second-brain (this project) at `.assistant/profile.yaml` (or path-TBD-by-T071) per the schema established in T071. The profile reflects /opt's actual needs: knowledge curation, methodology stewardship, source ingestion, lesson distillation, gateway-driven orientation, P4 verification discipline, sacrosanct-verbatim quoting. Serves as the canonical reference for sister-project profiles (M006).

## Done When

- [ ] `.assistant/profile.yaml` exists at /opt repo root (path per T071 outcome)
...
- [ ] Identity section: name=`opt-second-brain-assistant`, project=`devops-solutions-information-hub`, owner=`operator`
...
- [ ] Operator confirms profile reflects /opt's actual needs

## Relationships

- PART OF: [[e024-m003-opt-second-brain-assistant-profile-canonical-example|E024-M003 — /opt Profile]]
