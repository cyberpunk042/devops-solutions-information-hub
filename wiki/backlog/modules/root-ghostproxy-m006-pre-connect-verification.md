...
priority: P0
task_type: module
parent_epic: "root-ghostproxy-sfif-rollout-and-second-brain-integration-2026-05"
...
Before running `tools.setup --connect-project /root` (which lands as M007), verify the prerequisites are in place: Stream 2 M001 (AGENTS.md) is complete in /root because the connect script writes the `## Second Brain Connection` block INTO an existing AGENTS.md or CLAUDE.md; root-ghostproxy is at a clean git state so the connect script's mutations are reviewable as a single diff; operator authorizes the connection (the auto_connect=false flag in sister-projects.yaml is the explicit gate). M006 is short — checklist + sign-off — but skipping it is what causes connect-script failures and partial-state cleanups in /root.

## Done When

- [ ] /root/AGENTS.md exists (M001 dependency satisfied) — verified by file existence + line count
- [ ] /root/.git status is clean: no uncommitted changes that would be overwritten or that mix with the connect-script's diff
...
- [ ] Operator captures: any prior /root/.mcp.json content (so connect-script's MCP entry is added, not overwriting unknown state)
- [ ] Operator captures: any prior /root/tools/ Python content (so gateway.py + view.py forwarders are added without colliding)
...
> [!question] What if /root/.mcp.json already exists with content from prior sessions?
...
> [!question] What if /root/tools/gateway.py already exists?
...
> `python3 -m tools.setup --connect-project /root --dry-run` previews the four artefacts (.mcp.json mutation, gateway.py + view.py forwarders, AGENTS.md/CLAUDE.md brain-pointer block) without writing. Use this BEFORE the real run; review the diff. M006 includes the dry-run as a mandatory pre-step.

> [!question] Atomic rollback strategy?
> Best: a git commit pre-connect, so post-connect changes can be reverted as a single revert. Worst case: tar the relevant /root subtree. Operator picks the granularity.

## Tasks

| Task | Description | Status |
|---|---|---|
| T-M006-1 | Verify /root/AGENTS.md exists; record line count and frontmatter excerpt | ⊙ pending |
| T-M006-2 | `cd /root && git status` — confirm clean working tree; commit any pending work first | ⊙ pending |
| T-M006-3 | Capture /root/.mcp.json (if present) and /root/tools/ contents (if present) for collision-check | ⊙ pending |
...
- PART OF: [[root-ghostproxy-sfif-rollout-and-second-brain-integration-2026-05|Epic — root-ghostproxy SFIF Rollout]]
