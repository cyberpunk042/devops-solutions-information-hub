...
Design for upgrading the existing `gateway what-do-i-need` subcommand to be context-aware (brain / sister / external × fresh / task-bound) and SRP-compliant. This is NOT a new subcommand — it's a rework of an existing one. The current output mixes task routing + consumer-runtime signaling + default-profile advice (3 questions in 1 output, violating Rule 1 of the [[gateway-output-contract|Gateway Output Contract]]). This design separates concerns, adds context branches, and closes with an unambiguous next-move. Reuses M002's context detection (Decision 1), session-state (Decision 2), and warning protocol (Decision 4) — only the output modes are new.

## Goals

- SRP: what-do-i-need answers ONLY "given your context, what's the right work path?"
- Context-aware: brain-self returns knowledge-verb task types; sister returns sister routing; external returns MCP tool pointers
...
> |----------------------|----------------|---------------|
> | **PROJECT IDENTITY** section (domain, phase, scale, second brain, all ✓-marked) | No — contextual header, relevant to routing | KEEP — trimmed to 2 lines: `location: brain-self` + `freshness: task-bound` |
...
Two branches within brain-self based on freshness:
...
~30 lines. Under ceiling. SRP: routes to task types only. Knowledge-verb task types specific to the brain — NOT the app-dev models (feature-development, bug-fix, etc. are for sister projects).
...
WHAT DO YOU NEED? — You are a sister project connecting to the brain.

You need orientation before consuming.

NEXT: gateway orient    (learn how to consume the brain)
```

Same redirect pattern as brain-self + fresh. Keeps SRP.
...
~15 lines. Routes to brain queries + contribution flow. Keeps SRP (routing only; no teaching).
...
When what-do-i-need completes routing (brain-self + task-bound mode), it writes `current_task_type` to session-state. This enables subsequent orient invocations to detect `task-bound` freshness — closing the loop:
...
| T-E022-11 | Implement brain-self branch: fresh redirect + task-bound knowledge-verb table | implement |
...
> [!question] Should brain-self + task-bound include a "not finding your task type?" footer pointing to gateway orient?
...
> Per Decision 1: removed from main output. But should it move to orient (where it contextualizes the consumer's declarations) or to a new `gateway identity` subcommand? Lean: into orient's brain-self + task-bound redirect mode as a 1-line "Your declared runtime: <X>" — keeps orient's SRP (it contextualizes identity) without bloating it.
