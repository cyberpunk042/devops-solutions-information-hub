...
- [Adapters Never Raise — Failure As Data at Integration Boundaries](00_inbox/adapters-never-raise-failure-as-data-at-integration-boundaries.md) — At integration boundaries between a system and anything external — another process, a filesystem, a network call, a s...
- [Block With Reason and Justified Escalation — The Bypass Mechanism for Mindful Enforcement](00_inbox/block-with-reason-and-justified-escalation.md) — When an autonomous agent encounters a condition where it SHOULD not proceed silently — a blocked action, an out-of-sc...
- [Observe-Fix-Verify Loop — The Battle-Testing Cycle for Autonomous Agent Infrastructure](00_inbox/observe-fix-verify-loop.md) — The **Observe-Fix-Verify loop** (OFV) is the iteration cycle that hardens agent infrastructure through real operation...
...
`pattern`, `openfleet`, `enforcement`, `orchestration`, `cross-domain`, `deterministic`, `agent-compliance`, `claude-md`, `guardrails`, `harness`, `agent-architecture`, `mcp`, `feedback-loop`, `second-brain`, `infrastructure`, `quality`, `skills`, `adapter`, `receipt`, `failure-as-data`
