...
[[epic-readiness-math-is-wrong-when-an-epic-has-implicit-goals|Epic readiness math is wrong when an epic has implicit goals beyond its current children]]
...
[[per-task-cost-grows-monotonically-across-multi-task-runs-(co|Per-task cost grows monotonically across multi-task runs (context accumulation)]]
...
[[right-size-the-methodology-model-to-the-actual-work,-not-the|Right-size the methodology model to the actual work, not the structural category]]
