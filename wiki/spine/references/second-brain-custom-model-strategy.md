...
- **Purpose**: conversational assistant fluent in the 4 principles, 16 models, 26 standards. Answers "what does Principle 4 say?", drafts initial lesson scaffolds, suggests relationship links.
...
**The 5-candidate matrix above is tactical** — wiki-domain-fluency LoRAs for AICP routing + maintenance tasks. The 2026-05-04 directive layers **strategic-tier scope** on top, captured in the operator-authored [Custom-Tailored Senior-Engineer-Tier Model Group + Recreated Intelligence Layer Concept](../../domains/cross-domain/custom-tailored-senior-engineer-tier-model-group-with-recreated-intelligence-layer-research-synthesis.md) and [Epic](../../backlog/epics/pre-milestone/custom-tailored-senior-engineer-tier-model-group-with-recreated-intelligence-layer-2026-05.md). Six new dimensions extend (do not replace) the 5-candidate matrix:

| New dimension | What it adds | How it composes with the 5-candidate matrix |
|---|---|---|
| **Senior-engineer-tier core** | Behavioral target: model *naturally wants* to follow methodology over hack-and-rush | The 5 candidates remain valid; a senior-engineer-tier specialist LoRA becomes the **central** specialist in the model group. Empirical anchor: Qwen3.6-27B-Dense beats some 397B MoE on agentic coding (per Anti-Vendor-Lock-In Evidence 1). |
| **Behavioral-alignment core via preference data** | DPO/IPO/KTO over hack-vs-right output pairs; operator's standards as the alignment signal | Layered ON TOP of any of Candidates A–E. The Candidate B/C base + preference fine-tune is the highest-leverage combination. Empirical anchor: prime-rl trains RLM-Qwen3-8B in 48 H100 hours / ~$48–100 with IPO + Kimi-K2.5 KL default loss. |
| **MoE-style group of various sizes** | Mixture-of-LoRAs ([LoRAHub](https://arxiv.org/abs/2307.13269)) per task surface, optionally over an MoE base (Mixtral / DeepSeek V3 / Qwen3-30B-A3B / GPT-OSS shared-experts) | Candidate E (Multi-LoRA) generalizes to senior-engineer task surfaces (coding · methodology-reasoning · spec-authoring · validation-checking · refactor-planning · debug-analysis); routing layer chooses per workload. |
| **Recreated intelligence layer at I/O boundaries** | Input boundary: routing + Caveman compression + spec loading + context selection + tool-use planning. Output boundary: schema gate + self-verification + methodology compliance + hallucination detection | Wiki-Router (Candidate D) becomes the input-boundary router; output-boundary intelligence is new. Python-as-programming hyperstructure on top of [Markdown-as-IaC](../models/agent-config/model-markdown-as-iac.md). |
| **Multi-version release discipline + information-virus propagation** | Per-version manifest (base + LoRA + preference data + instruction data + constitution + composition); maturity ladder (`v0.1-seed` → `v0.5-growing` → `v1.0-validated`); sister-project consumer integration spreads operator's standards by weight | The wiki's evolution lifecycle (00_inbox → 04_principles) IS the precedent. Each released model version is a contributable artifact for sister projects (OpenArms · OpenFleet · AICP · devops-control-plane · root-ghostproxy when registered). |
| **Composition with trust + compression layers** | Operator-toggle-able L0–L4 opt-ins per workload; default L2 on RTX 4090 (compressed-encrypted weights + on-GPU decypher via Triton + Caveman input-prompt compression); operator-decision per workload | Composes orthogonally with the existing 5-candidate matrix — any candidate runs at any L0–L4 stance with/without I/O compression. |

**Pain-point root cause (operator-named 2026-05-04):** fresh-environment session-start alignment overhead (CLAUDE.md + AGENTS.md + .claude/rules/ + harness + ecosystem config repeatedly re-paid per session) is the *symptom*; the *cause* is that the alignment substrate is external to the model. Baking operator's standards into the weights via preference fine-tune + curated instruction data + behavioral constitution is the **solution at the right place** — pay alignment cost once at training, not per session at every fresh environment.

**Strategic-tier reshape of the phased rollout (2026-05-04):**

| Phase | What it adds on top of the 2026-04-22 phased rollout |
|---|---|
| **Phase 1.5 — Constitution + preference data v0.1** | Operator-curated hack-vs-right preference pairs + behavioral constitution; the alignment signal that distinguishes operator-tier from base-tier |
| **Phase 4 (was Candidate E)** | Generalizes from Multi-LoRA-per-domain to Mixture-of-LoRAs-per-senior-engineer-task-surface; the model group with size variance |
| **Phase 5 — Recreated intelligence layer at I/O boundaries** | Python hyperstructure: input/output boundary intelligence; previously implicit, now explicit infrastructure |
| **Phase 6 — Behavioral preference fine-tune** | DPO/IPO over operator-curated preference pairs; the highest-leverage step; what makes the model behaviorally distinct from base |
| **Phase 7 — Trust + compression composition + ecosystem propagation** | L0–L4 opt-ins · multi-version release · sister-project integration · pain-point reduction empirically measured |

**The deeper principle layered on the 2026-04-22 deeper principle**: the custom-model strategy is not just *gap-filling in the commercial landscape* — it is also *root-cause solution to the per-session alignment overhead*. When operator's standards are external to the model, every fresh environment pays alignment cost; when operator's standards are in the weights, alignment is a one-time training cost. The strategic-tier scope is the operator's recognition that the per-session repetition is unsustainable at scale.

## Data Preparation Strategy

The wiki corpus as of 2026-04-28: 525 pages, 3,298 relationships, 16 models, 26 standards, 41 validated lessons, 15 validated patterns, 16 validated decisions, 77 source syntheses, 4 principles. All under consistent YAML frontmatter + Markdown structure.
...
- **Standards** (26 pages) → Q/A on "what does good look like for X" — ~10 each = ~260
