...
| **Composition with trust + compression layers** | Operator-toggle-able L0–L4 opt-ins per workload; default L2 on RTX 3090 (compressed-encrypted weights + on-GPU decypher via Triton + Caveman input-prompt compression); operator-decision per workload | Composes orthogonally with the existing 5-candidate matrix — any candidate runs at any L0–L4 stance with/without I/O compression. |
