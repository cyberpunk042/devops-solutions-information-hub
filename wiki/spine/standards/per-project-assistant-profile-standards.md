...
sources:
  ...
  - id: opt-canonical-profile
    type: wiki
    file: wiki/domains/cross-domain/opt-second-brain-assistant-profile-the-canonical-tool-agnostic-per-project-assistant-definition.md
    description: "The /opt second-brain's own canonical Profile — exemplar of standards application"
  - id: spectrum-concept
    type: wiki
...
2. **A Profile is per-project, authored by that project.** /opt does NOT author OpenArms's Profiles or OpenFleet's Profiles or AICP's Profiles. Each project authors its own Profiles in its own repo. /opt second-brain hosts the **standards** + **model** + **integration** + **super-model** that ALL projects consume. Cross-project Profile-authoring at /opt is a boundary violation.
...
| **2** | **Knowledge Scope** | Bounds what the assistant can/should know | brain_files (L0 layer) · project_scope (paths in this project's repo) · second_brain_references (/opt content this Profile references — for consumer projects) · external_references (sister-project cross-links) · forbidden_scope (what NOT to bring into the assistant's knowledge) |
...
| Who authors the Profile schema, standards, model, super-model? | /opt second-brain (this project) — uniquely. |
| Can /opt author sketches of sister-project Profiles? | NO. That's a cross-project boundary violation. Each project authors its own. |
| Can /opt provide a TEMPLATE that sister projects use? | YES. Profile templates live at `wiki/config/templates/` and are operator-shipped. |
...
| Where do /opt's own Profiles live? | At /opt's wiki: `wiki/domains/cross-domain/opt-profile-<job-name>.md` (one file per focused job — e.g., `opt-profile-continuous-research-...md`, `opt-profile-pipeline-synthesis-...md`). |
| Can a Profile reference /opt content? | YES via Knowledge Scope. Sister projects CONSUME from /opt; /opt does not CREATE FOR them. |
...
|---|---|---|
| /opt authors sister-project Profiles | Authoring "OpenArms Assistant Profile" at /opt | Delete. Each project authors its own. /opt provides standards + model + integration + super-model. |
...
1. **Read these standards** + read /opt's canonical Profile as an exemplar
...
7. **Optionally cross-reference** from /opt second-brain via gateway contribute (lands in 00_inbox)

### How /opt Maintains These Standards
...
## Cross-references to other /opt-side meta-layer artifacts

The Per-Project Assistant Profile Standards is **one of four** /opt-side meta-layer artifacts (operator-named 2026-05-09 turn 5: "standards and the model and integration into the knowledge and super-models"). The others (planned):
...
These four together comprise /opt's unique contribution to the Profile ecosystem. Other projects (OpenArms, OpenFleet, AICP, dcp, OpenClaw, root-ghostproxy) author their own Profiles consuming these standards/model/integration/super-model artifacts.
...
- BUILDS ON: [[opt-second-brain-assistant-profile-the-canonical-tool-agnostic-per-project-assistant-definition|/opt Second-Brain Assistant Profile]] (exemplar of standards application)
...
[[/opt Second-Brain Assistant Profile]]
...
[[profile-integration-into-the-knowledge-cross-reference-topology-with-existing-wiki-layers|Concept — Profile Integration into the Knowledge: cross-reference topology between Per-Project Assistant Profiles and existing /opt wiki layers (lessons · patterns · decisions · models · standards · super-model)]]
...
[[e024-m006-cross-project-profile-catalog-everywhere-integration|E024-M006 — /opt-side Meta-Layer for Per-Project Profiles (standards · model · integration · super-model) — the unique /opt contribution; sister projects author their OWN Profiles consuming this meta-layer]]
[[model-per-project-assistant-profile|Model — Per-Project Assistant Profile]]
[[opt-profile-continuous-research-keep-models-and-tech-vision-current|Profile — /opt Continuous Research: focused assistant Profile for keeping models + technology-vision current; runnable as a 24/7 OpenClaw (or other tool) agent]]
[[opt-profile-pipeline-synthesis-from-raw-to-wiki-page-still-not-at-end-of-pipeline|Profile — /opt Pipeline Synthesis: focused assistant Profile for synthesizing ingested information still not at end of pipeline (raw → wiki page); runnable as a 24/7 OpenClaw (or other tool) agent]]
