...
sources:
  ...
  - id: operator-correction-2026-05-09-turn-5
    ...
    title: "2026-05-09 turn 5 correction — every project does own Profiles; /opt has standards + model + integration + super-models"
  ...
  - id: opt-profile-continuous-research
    type: wiki
    file: wiki/domains/cross-domain/opt-profile-continuous-research-keep-models-and-tech-vision-current.md
    title: "Profile — /opt Continuous Research (canonical example #1)"
  - id: opt-profile-pipeline-synthesis
    type: wiki
    file: wiki/domains/cross-domain/opt-profile-pipeline-synthesis-from-raw-to-wiki-page-still-not-at-end-of-pipeline.md
    title: "Profile — /opt Pipeline Synthesis (canonical example #2)"
  - id: spectrum-concept
    type: wiki
...
| Can /opt author sister-project Profiles? | NO — boundary violation. Each project authors its own. |
| Who authors the standards / model / integration / super-model? | **/opt second-brain uniquely** (this is /opt's contribution beyond its own Profiles) |
...
|---|---|---|
| [[opt-profile-continuous-research-keep-models-and-tech-vision-current\|/opt Continuous Research]] | wiki/domains/cross-domain/ | Keep models + technology-vision current (operator-named example #1) |
| [[opt-profile-pipeline-synthesis-from-raw-to-wiki-page-still-not-at-end-of-pipeline\|/opt Pipeline Synthesis]] | wiki/domains/cross-domain/ | Synthesize ingested information still not at end of pipeline (operator-named example #2) |
| (more /opt Profiles per operator-naming, with "things like this...") | wiki/domains/cross-domain/ | TBD |
| Per-sister-project Profiles (OpenArms · OpenFleet · AICP · OpenClaw · root-ghostproxy · etc.) | Each project's OWN repo | NOT /opt's authoring scope |
...
Profile authored (project owner)
   ↓
Profile referenced from /opt (standards + model + integration applied)
   ↓
...
| /opt authoring sister-project Profiles | Cross-project boundary violation (turn 5) |
...
### Why this Model is at /opt and not in sister projects

Operator-doctrinal 2026-05-09 turn 5 (sacrosanct): *"the difference in the second-brain is that we do not only have profiles for AI assistants but we will also have the standards and the model and integration into the knowledge and super-models"*. The Profile **model** (this page) is one of four /opt-unique meta-layer artifacts; sister projects consume from /opt, don't host these artifacts. This is /opt's contribution to the ecosystem.
...
- INSTANCES: [[opt-profile-continuous-research-keep-models-and-tech-vision-current|/opt Continuous Research]] · [[opt-profile-pipeline-synthesis-from-raw-to-wiki-page-still-not-at-end-of-pipeline|/opt Pipeline Synthesis]]
