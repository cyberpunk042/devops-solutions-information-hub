...
- Does `notebooklm-py` support programmatic source insertion at scale (250+ sources in one command), or does it require sequential API calls that may rate-limit? (Requires: empirical testing of notebooklm-py source batch insertion; the notebooklm-py CLI page documents rate limiting as a known risk but does not document whether batch-level or sequential insertion is supported)
- Can NotebookLM notebooks be shared across team members and queried by multiple Claude Code instances simultaneously? (Requires: external testing of NotebookLM sharing and concurrent access behavior; the notebooklm-py CLI page documents multi-profile support but not concurrent multi-user access semantics)

### Answered Open Questions

**Q: What is the latency cost of a NotebookLM query via `notebooklm-py` CLI vs. a local wiki search? Is it fast enough for in-session use or better suited for pre-session research preparation?**

Cross-referencing `notebooklm-py CLI` and `Context-Aware Tool Loading`: the notebooklm-py CLI page documents that the package "uses Playwright/Patchright for browser automation since no official NotebookLM API exists." Browser-automation-based queries carry inherent latency overhead — launching a browser session, authenticating, submitting the query, and receiving the response. Additionally, the page notes "session state: browser automation sessions can expire, requiring re-authentication. Long-running daemons need session refresh logic." The Context-Aware Tool Loading pattern directly addresses this latency consideration: "External services with high query latency where on-demand fetching creates unacceptable wait times in interactive workflows. Pre-fetching at session start may be the better UX tradeoff." This confirms the wiki's own architecture conclusion: local wiki search (file reads, manifest queries) is zero-latency and suitable for mid-session lookups; NotebookLM queries via notebooklm-py are better suited for pre-session research preparation or asynchronous pipeline steps where latency is acceptable. The notebooklm-py CLI page's "cross-validation" use case (query sources to validate wiki page claims) implies batch pre-session use, not interactive in-session querying.

**Q: How does the notebooklm-py skill compare to a NotebookLM MCP server for this integration pattern? Has anyone built an MCP wrapper for notebooklm-py?**
