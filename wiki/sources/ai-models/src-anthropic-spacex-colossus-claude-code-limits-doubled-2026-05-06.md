...
6. **Demand-side composition is shifting toward multi-agent workflows** — Ars cites "a user behavior (and product) shift away from single-agent, chat-based tasks to more demanding multi-agent workflows" as a primary driver of the demand spike. This **independently corroborates** the agent-as-default-interaction-surface hypothesis embedded in this project's [[three-layer-agent-context-architecture|Three-Layer Agent Context Architecture]] and the [Managed Agents v2](../sources/ai-models/) thesis: Anthropic's own demand telemetry is showing chat-as-shell receding while multi-agent-as-shell rises.
