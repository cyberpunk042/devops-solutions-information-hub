...
>
> Per the [[end-to-end-compression-across-the-ai-stack-composes-multiplicatively-6-plus-independent-mechanisms-at-6-distinct-layers|Multi-Layer Compression Convergence Lesson]], Layer 6 (KV-cache + internal representation) substitutable axes were: KV-cache asymmetric quantization (50-87%) · Qwen-Scope SAE sparse-feature representation (top-k of 16×–64× hidden size) · attention sparsity · sliding-window attention. **DSA adds two more sub-mechanisms**:
>
...
Per [[anti-vendor-lock-in-is-an-empirical-claim-when-every-stack-layer-has-paper-evidence|Anti-Vendor-Lock-In Lesson]]: every stack layer needs substitutable axes with paper evidence. DeepSeek V4 adds:
