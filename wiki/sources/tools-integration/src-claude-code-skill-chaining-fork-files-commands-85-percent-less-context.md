...
>
> **Operator-mission alignment**: [[goldilocks-protocol|Goldilocks Protocol]] — pick optimization scope per workload. The 85% reduction is mission-relevant for high-frequency or large-context workloads; not worth the engineering for one-shot lookups.
...
[[end-to-end-compression-across-the-ai-stack-composes-multiplicatively-6-plus-independent-mechanisms-at-6-distinct-layers|Multi-Layer Compression Lesson]] lists 12 mechanisms across 6 layers + cross-cutting paradigms. **This synthesis adds a CROSS-LAYER technique** that operates at three layers simultaneously:
...
Both are Layer-3 (Tool I/O) substitutable axes per [[anti-vendor-lock-in-is-an-empirical-claim-when-every-stack-layer-has-paper-evidence|Anti-Vendor-Lock-In Lesson]]. They compose: a skill with intent-based tools running in a forked context with file handoff would compound both reductions.
