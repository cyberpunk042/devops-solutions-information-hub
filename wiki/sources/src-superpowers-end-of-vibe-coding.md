...
- Is the spec document format standardized, or does it vary per brainstorm session? A consistent spec schema would enable automated plan generation. (Requires: external documentation of the Superpowers spec format; no wiki page captures a standardized spec schema from the Superpowers plugin)
- What is the typical session cost (tokens + time) for a full Superpowers cycle on a medium-complexity feature? Is it practical for small bug fixes or only for multi-day features? (Requires: empirical data from real Superpowers workflow runs; no wiki page documents observed token or time costs for a complete cycle)

### Answered Open Questions

**Q: How does the 11-task sub-agent approach handle cross-task dependencies — if task 3 produces an API that task 7 consumes, how does the orchestrator communicate the API shape between sub-agents?**

Cross-referencing `Plan Execute Review Cycle` and `Agent Orchestration Patterns`: the Plan Execute Review Cycle page describes the Superpowers plugin's externalization mechanism: "Plans and todos serve a structural function beyond task management: they are the boundary object between the plan phase and the execute phase. Once a plan is written and approved, it constrains what the execute phase can legitimately do." The implementation for cross-task dependencies is the spec document and the plan file: during the Planning phase, the implementation plan captures the API shape produced by task 3 (e.g., the function signatures, data types, and interface contracts) as part of task 3's output specification. Task 7's input spec in the same plan file references this output. When the task 7 sub-agent is dispatched, it receives the plan file — which contains the documented API contract from task 3 — as context. The Agent Orchestration Patterns page confirms the mechanism: "Initialize fresh context — do not pass the full conversation history. Pass only what is required for the delegated task." The plan file is the "only what is required" artifact. Deviations where task 3 produces a different API than planned are surfaced by the code review agent phase, which "found critical and important issues" by reviewing the full implementation against the spec — catching API contract mismatches before merge.

**Q: How does Superpowers compare to OpenFleet's sub-agent dispatch model for multi-agent task execution?**
