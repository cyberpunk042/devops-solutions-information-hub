...
Source: https://arxiv.org/pdf/2603.28052v1
Ingested: 2026-04-15
Type: paper
...
**Authors:** Lee, Yoonho, Nair, Roshen, Zhang, Qizheng, Lee, Kangwook, Khattab, Omar, Finn, Chelsea
...
Code
27.5
T erminalBench-2 Harness Performance
Human-written
...
2, Meta-Harness outperforms all reportedClaude Haiku 4.5harnesses.
Abstract
The performance of large language model (LLM) systems depends not
only on model weights, but also on theirharness: the code that determines
...
short templates or summaries. We introduceMeta-Harness, an outer-loop
...
performance gap on the same benchmark [47]. Theharness—the code that determines what
to store, retrieve, and show to the model—often matters as much as the model itself. This
sensitivity has led to growing interest inharness engineering, the practice of refining the
...
Harness Code
1 2
St or e all Logs t o Filesyst em
T asks
Ev aluat e
Pr oposed 
Code
...
Meta-HarnessFullalllogs and scores 10.0
...
from one evaluation of a text artifact in thelargest setting considered in each paper. This paper
...
We address this limitation withMeta-Harness, an agentic harness for optimizing harnesses
...
the proposer must decidewhatto inspect and validate edits through direct interaction with
the codebase. Its key design choice is to exposefull historythrough afilesystem, enabling
...
In practice, the proposer reads a median of82 files per iterationin our most demanding
...
Agentic Context Engineering (ACE, Zhang et al. [59]) by7.7 pointswhile using 4 × fewer
...
harness improves accuracy on 200 IMO-level problems by4.7 pointson average across five
held-out models. On TerminalBench-2, the discovered harnesssurpasses Terminus-KIRA
...
harnessto refer to the task-specific programs being optimized.
...
reward function r(τ, x) scores the trajectory. The objective of harness optimization is tofind
the harness that maximizes the expected final reward:
H∗ =arg max
H
Ex∼X,τ∼p M(H,x) r(τ,x),
...
to a growing filesystem D that serves as its feedback channel 1. Here, acoding agentis a
...
imposes no parent-selection rule: the proposer is free to inspectanyprior harness and its
...
The proposer never sees test-set results; its only feedback comes from thesearch set, the
...
1Based on earlier exploration, we think this workflow only became practical recently, following
major improvements in coding-agent capabilities around early 2026.
4

## Page 5

Algorithm 1Meta-Harness outer loop over harnesses
1:Input:tasksX, LLMM, proposerP, iterationsN
...
6:D ← D ∪ {(H,EH)}
7:fort=1 . . .Ndo
8:ProposerPqueries filesystemD▷inspects prior harnesses and scores
9:ProposerPproposesknew harnesses{H 1, . . . ,Hk}
10:forHin{H 1, . . . ,Hk}do
11:ifHpasses interface validationthen
12:D ← D ∪ {(H, EVALUATE(H,M,X))}
13:returnPareto frontier of harnesses stored inD
traces, the proposer can often inferwhya harness failed and which earlier design choices
likely contributed to the failure, not justthatit failed, as illustrated by the search trajectories
...
ACE [59]† 16.077.8 29.0 40.9 50.8
Meta-Harness14.086.8 45.0 48.611.4
...
(22 classes); andUSPTO-50k[ 41] predicts precursor reactants from product molecules
...
Discover) in0.1 × the evaluations, and its final accuracy surpasses theirs by more than 10
...
Meta-Harness preservesfull experience history using a filesystemand allows the proposer to
...
Few-shot (8) 34.0 63.0 54.0 90.0 44.0 82.7 84.791.376.7 68.9 2.2
Few-shot (32) 38.7 62.0 53.3 90.7 43.386.085.3 90.7 76.7 69.6 5.2
Few-shot (all) 35.3 61.0 50.0 93.3 42.7 80.7 84.0 90.0 76.7 68.2 7.4
ACE [59] 40.774.048.096.744.0 83.3 86.0 90.7 68.7 70.2 11.7
Meta-Harness53.367.060.094.046.082.786.7 91.3 77.3 73.17.3
...
MethodGPT-5.4n GPT-5.4m Gem-3.1FL Gem-3F GPT-20B Avg.
...
Meta-Harness31.7(+8.7)30.4(+1.6)34.9(+6.3)46.3(+3.7)50.6(+3.0) 38.8(+4.7)
...
baseline across all five held-out models, with an average gain of4.7 points. It also matches
...
evaluation on the same 89-task benchmark. We use this benchmark as adiscovery problem[54]
...
2. Results or others are from the offi-
cial leaderboard.Meta-Harness ranks
#2 among all Opus-4.6 agents and #1
among all Haiku-4.5 agents on this
competitive task.
...
Opus 4.6 agents on the TerminalBench-2 leader-
board. The only higher-scoring Opus 4.6 agent is
...
search trajectory helps explainwhyMeta-Harness
...
thatfilesystem access enables the proposer to inspect prior experience in enough detail to
...
advantage of Meta-Harness is not just search over code, but search withselective access to
...
[2] Ekin Aky ¨urek, Dale Schuurmans, Jacob Andreas, Tengyu Ma, and Denny Zhou. What
...
[6] Mislav Balunovi´c, Jasper Dekoninck, Ivo Petrov, Nikola Jovanovi´c, and Martin Vechev.
...
[9] Birgitta B¨ockeler. Harness engineering. https://martinfowler.com/articles/explor
ing-gen-ai/harness-engineering.html, March 2026. martinfowler.com.
[10] Can B¨ol ¨uk. I improved 15 LLMs at coding in one afternoon. only the harness changed.
https://blog.can.ac/2026/02/12/the-harness-problem/, February 2026.
[11] I ˜nigo Casanueva, Tadas Temˇcinas, Daniela Gerz, Matthew Henderson, and Ivan Vuli´c.
...
marking legal knowledge of large language models. InProceedings of the 2024 conference
...
adaptation of deep networks. InInternational Conference on Machine Learning, 2017.
...
[22] Phillip Keung, Yichao Lu, Gy ¨orgy Szarvas, and Noah A. Smith. The multilingual
amazon reviews corpus, 2020. URLhttps://arxiv.org/abs/2010.02573.
...
optimization via pairwise comparison. InarXiv preprint arXiv:2511.07919, 2025.
...
entity recognition for xbrl tagging. InProceedings of the 60th Annual Meeting of the
...
matical reasoning. InProceedings of the 2025 Conference on Empirical Methods in Natural
Language Processing, 2025. URLhttps://aclanthology.org/2025.emnlp-main.1794/.
...
[35] Alexander Novikov, Ngˆan V ˜u, Marvin Eisenberger, Emilien Dupont, Po-Sen Huang,
...
[40] Jurgen Schmidhuber. A neural network that embeds its own meta-levels. InIEEE
...
[42] Srijan Shakya, Anamaria-Roberta Hartl, Sepp Hochreiter, and Korbinian P ¨oppel.
...
large language model agents develop industry-level mobile applications? InarXiv
...
retrieval benchmark, 2024. URLhttps://arxiv.org/abs/2404.06347.
[50] Yiming Xiong, Shengran Hu, and Jeff Clune. Learning to continually learn via meta-
learning agentic memory designs. InOpenReview, 2026. URL https://api.semanticsc
holar.org/CorpusID:285454009.
[51] Chengrun Yang, Xuezhi Wang, Yifeng Lu, Hanxiao Liu, Quoc V Le, Denny Zhou,
and Xinyun Chen. Large language models as optimizers. InThe Twelfth International
...
discover at test time, 2026. URLhttps://arxiv.org/abs/2601.16175.
...
improving language models. InarXiv preprint arXiv:2510.04618, 2025.
[60] Xiang Zhang, Junbo Zhao, and Yann LeCun. Character-level convolutional networks
for text classification, 2016. URLhttps://arxiv.org/abs/1509.01626.
14
...
evo strip onlybase and proposes a systems-level optimization:
...
but that the proposer articulateswhyit should be safer: it avoids touching the previously
...
same memory conditioned on that draft label, retrieving 5confirmerswith the same label
and 5challengerswith different labels, and asks the model whether to maintain or revise its
...
It begins with alabel primerlisting the valid output labels, then constructs acoveragesection
with one query-relevant example per label, and finally addsquery-anchored contrastive pairs
...
Meta-Harness (Draft Verification)18.085.4 17.0 40.1 5.4
...
Meta-Harness (RRF + Contrastive)18.089.6 35.0 47.5 41.4
Meta-Harness (Relevance + Contrastive)18.0 90.636.0 48.2 43.9
Meta-Harness (Label-Primed Query)14.0 86.845.0 48.645.5
...
Each pink dot is a discovered strategy; baselines are labeled. The dashed diagonal isy=x.
...
• SciCiteis a 3-way citation-intent classification benchmark introduced by Cohan et al.
...
• FiNER-139is a financial numeric entity recognition benchmark introduced by Loukas
...
• Amazon Reviewsis the English portion of the Multilingual Amazon Reviews Corpus
...
• Financial PhraseBankis a 3-way financial sentiment benchmark introduced by Malo
...
• GoEmotionsis a fine-grained emotion classification benchmark introduced by Demszky
...
• Banking77is a fine-grained intent classification benchmark introduced by Casanueva
...
• AG Newsis a 4-way news topic classification benchmark commonly associated with
...
• SciTailis a science-domain textual entailment benchmark in which the task is to predict
...
andMeta-Harnessacross the five held-out models.
23
...
is relevant. Meta-Harness gives the proposer access toallprior candidates simultaneously
...
level: it searches over theimplementationof these policies in executable code, treating the
