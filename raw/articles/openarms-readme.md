...
Source: /home/jfortin/openarms/README.md
Ingested: 2026-04-08
Type: documentation
...
<p align="center">
    <picture>
        <img src="./assets/openarms.png" alt="OpenArms" width="500">
    </picture>
</p>

<p align="center">
  <strong>FREEDOM! PRIVACY!</strong>
</p>

<p align="center">
  <a href="https://github.com/cyberpunk042/openarms/actions/workflows/ci.yml?branch=main"><img src="https://img.shields.io/github/actions/workflow/status/cyberpunk042/openarms/ci.yml?branch=main&style=for-the-badge" alt="CI status"></a>
  <a href="https://github.com/cyberpunk042/openarms/releases"><img src="https://img.shields.io/github/v/release/cyberpunk042/openarms?include_prereleases&style=for-the-badge" alt="GitHub release"></a>
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-MIT-blue.svg?style=for-the-badge" alt="MIT License"></a>
</p>

**OpenArms** is a _personal AI assistant_ you run on your own devices.
...
Preferred setup: run `openarms onboard` in your terminal.
OpenArms Onboard guides you step by step through setting up the gateway, workspace, channels, and skills. It is the recommended CLI setup path and works on **macOS, Linux, and Windows (via WSL2; strongly recommended)**.
...
```bash
npm install -g openarms@latest
# or: pnpm add -g openarms@latest

openarms onboard --install-daemon
```

OpenArms Onboard installs the Gateway daemon (launchd/systemd user service) so it stays running.
...
openarms gateway --port 18789 --verbose

# Send a message
openarms message send --to +1234567890 --message "Hello from OpenArms"
...
openarms agent --message "Ship checklist" --thinking high
```

Upgrading? [Updating guide](https://docs.openclaw.ai/install/updating) (and run `openarms doctor`).
...
Switch channels (git + npm): `openarms update --channel stable|beta|dev`.
...
```bash
git clone https://github.com/cyberpunk042/openarms.git
cd openarms
...
pnpm openarms onboard --install-daemon

# Dev loop (auto-reload on source/config changes)
pnpm gateway:watch
```

Note: `pnpm openarms ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `openarms` binary.

## Security defaults (DM access)

OpenArms connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.
...
- Approve with: `openarms pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`).

Run `openarms doctor` to surface risky/misconfigured DM policies.
...
OpenArms can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
- `gateway.bind` must stay `loopback` when Serve/Funnel is enabled (OpenArms enforces this).
...
- Controlled via `openarms nodes …`.

Runbook: [iOS connect](https://docs.openclaw.ai/platforms/ios).

### Android node (optional)

- Pairs as a WS node via device pairing (`openarms devices ...`).
- Exposes Connect/Chat/Voice tabs plus Canvas, Camera, Screen capture, and Android device command families.
- Runbook: [Android connect](https://docs.openclaw.ai/platforms/android).

## Agent workspace + skills

- Workspace root: `~/.openarms/workspace` (configurable via `agents.defaults.workspace`).
- Injected prompt files: `AGENTS.md`, `SOUL.md`, `TOOLS.md`.
- Skills: `~/.openarms/workspace/skills/<skill>/SKILL.md`.

## Configuration

Minimal `~/.openarms/openarms.json` (model + defaults):
...
- Link the device: `pnpm openarms channels login` (stores creds in `~/.openarms/credentials`).
...
- Official Tencent plugin via [`@tencent-weixin/openclaw-weixin`](https://www.npmjs.com/package/@tencent-weixin/openclaw-weixin) (iLink Bot API). Private chats only; v2.x requires OpenArms `>=2026.3.22`.
- Install: `openarms plugins install "@tencent-weixin/openclaw-weixin"`, then `openarms channels login --channel openclaw-weixin` to scan the QR code.
...
OpenArms was built for **Molty**, a space lobster AI assistant. 👐
by Peter Steinberger and the community.

- [openclaw.ai](https://openclaw.ai)
- [soul.md](https://soul.md)
- [steipete.me](https://steipete.me)
- [@openarms](https://x.com/openarms)
